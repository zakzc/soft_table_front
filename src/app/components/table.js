...
export default function Table(props) {
  ...
  const [numberOfPages] = useState(Math.round(list.length / 3));
  const [people] = useState(list.slice(pageBegin, pageEnd));
  console.log("-->", people, numberOfPages);

  // views
  const Pages = () => {
    return (
      <p className="z-10 bg-indigo-50 border-indigo-500 text-indigo-600 relative inline-flex items-center px-4 py-2 border text-sm font-medium">
        a
      </p>
    );
  };
        ...
        <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
          <div>
            <p className="text-sm text-gray-700">
              Showing <span className="font-medium">{pageBegin + 1}</span> to{" "}
